...
-- pattern matching, recursion and QuickCheck.  We will be grading
...
--
-- The installation of Haskell on 'corn' (accessible using 'ssh
-- sunetid@corn.stanford.edu') has QuickCheck preinstalled and this is
...
-- you can install QuickCheck using 'cabal install QuickCheck-2.6'
--
...
--
-- Please implement these three predicates.  You can then test your
-- program by writing 'check prop_optimizer_correctness' (or another
-- property) in your GHCi session; you can also test the correctness of
-- your properties by introducing bugs to the implementation and seeing
-- if the properties fail.
...
--
--      PUSH 1; PUSH 2; OP +; PUSH 3; OP *
--
-- After running the first two instructions, we have a stack consisting
-- of [2, 1].  Running the plus OP consumes these two values and
...
decompileInstr (IOp b) (x:y:rest) = Just (Op b x y:rest)
